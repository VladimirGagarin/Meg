...
export default function SongScreen() {
 ...
 const audioRef = useRef(null);
  ...
  useEffect(() => {
    if (!audioRef.current || !currentSongObj) return;
    
    const audio = audioRef.current;
    if (!audio) return; // Wait until audio is mounted

     audio.src = currentSongObj.audio;
    ...
    audio.addEventListener("playing", () => {
      ...
      // pause audio if the modal is open
      if (showModal) {
        audio.pause();
      }
    });
    ...
    audio.addEventListener("pause", () => { 
      setAudioStalled(false); // Reset stalled state when paused
    });
    ...
    return () => {
      ...
      audio.removeEventListener("pause", () => setAudioStalled(false));
      audio.removeEventListener("timeupdate", () => {
        // Cleanup timeupdate listener if needed
      });
      audio.removeEventListener("canplaythrough", () => setIsAudioReady(true));
      audioRef.current = null; // Clear the ref to avoid memory leaks
      setCurrentSongObj(null); // Reset current song object
      setCurrentLyricIndex(0); // Reset lyric index
      setCurrentImage(LogoImage); // Reset image to default
      setIsAudioReady(false); // Reset audio ready state
      setAudioStalled(false); // Reset audio stalled state
      setshowSonnetModalConfim(false); // Reset sonnet modal confirm state
      setshowSonnetModal(false); // Reset sonnet modal state
      setIsPlayingSonnet(false); // Reset sonnet playing state
      setSonnetLyricIndex(0); // Reset sonnet lyric index
  ...
  return (
    <div className="song-screen" ref={songScreenRef}>
      <audio
        ref={audioRef}
        preload="auto"
        style={{ display: "none" }} // or className="hidden-audio"
      />
      ...
      {isAudioReady && <Controls audio={audioRef} hideNext={false} id={id} />}
