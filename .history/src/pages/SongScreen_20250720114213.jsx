...
export default function SongScreen() {
  ...
  const closeSonnetModal = () => { setShowSonnetModal(false) }
  const handleResumePrevious = () => {
    if (!previousSongObj) return;

    setCurrentSongObj(previousSongObj);

    const audio = audioRef.current;
    audio.pause();
    audio.src = previousSongObj.audio;
    audio.load();
    audio.play();

    setPreviousSongObj(null); // Clear after restoring
  };

  
  const handleSonnetsong = () => {
    closeSonnetModal();
    setPreviousSongObj(currentSongObj); // Save previous song before override
    const sonnet = {
      id: "sonnet_song",
      title: "The Sonnet",
      audio: SonnetSong,
    ...
    setCurrentSongObj(sonnet);

    const audio = audioRef.current;
    audio.pause();
    audio.src = SonnetSong;
    audio.load();
    audio.play();

     audio.addEventListener("ended", handleResumePrevious, { once: true });
  };
      ...
      {showSonnetModal && (
        <div className="modal-overlay">
          <div className="sonnet-modal">
            ...
            <p>Would you like to reveal this sonnet’s heart and melody?</p>
            <div className="modal-buttons">
              <button className="yes-btn" onClick={handleSonnetsong}>
                Yes, unveil it
              </button>
              <button className="no-btn" onClick={closeSonnetModal}>
                No, not now
              </button>
            </div>
          </div>
