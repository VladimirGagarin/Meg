...
export default function SongScreen() {
 ...
 const audioRef = useRef(null);
  ...
  useEffect(() => {
    const audio = audioRef.current;
    let stoppedTime = 0; // Variable to store the stopped time
    if (!audio) return;

    if (showModal) {
    stoppedTime = audio.currentTime;
      // Pause the audio when the modal is shown
      audio.pause();
    } else {
      // Only resume if it was previously playing
      audio.currentTime = stoppedTime; // Ensure current time is set
