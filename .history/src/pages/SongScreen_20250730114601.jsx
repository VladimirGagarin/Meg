...
export default function SongScreen() {
  ...
  const stoppedTimeRef = useRef(0); // Persist across renders
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      audio.removeEventListener("pause", () => setIsAudioReady(true));
      audio.removeEventListener("canplaythrough", () => setIsAudioReady(true));
    };
  }, [currentSongObj, id, showModal]);
  ...
  useEffect(() => {
    ...
    if (showModal) {
      ...
      stoppedTimeRef.current = audio.currentTime;
      audio.pause();
    } else {
      // Restore time and resume
      audio.currentTime = stoppedTimeRef.current;
  ...
  if (!currentSongObj) {
    return <div className="loading"><spanJust a moment...</div>;
  }
    ...
    <div className="song-screen" ref={songScreenRef}>
      ...
      <div className="image-container" onDoubleClick={() => setShowModal(true)}>
        <img
          ...
          style={{ filter: greyScale ? "grayscale(100%)" : "none" }}
