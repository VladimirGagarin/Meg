  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      ...
      let newIndex = currentSongObj.lyrics.findIndex((line, i) => {
        ...
        return currentTimeMs >= line.start && (!next || currentTimeMs < next.start);
      });
        ...
        <div className="modal-overlay">
          <div className="sonnet-modal">
            ...
            <div className="modal-buttons">
              <button className="yes-btn" onClick={handleSonnetsong}>Yes, unveil it</button>
              <button className="no-btn" onClick={() => setshowSonnetModalConfim(false)}>No, not now</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="Sonnet-overlay">
          <div className="Sonnet-content">
            <div className="Controls">
              <button onClick={toggleSonnetPlay}>{isPlayingSonnet ? <FaPause /> : <FaPlay />}</button>
              <button onClick={closeSonnetModal}><FaTimes /> Stop</button>
            </div>
            <div className="lyrics-sonnet">
              {SonnetLyrics()[currentSonnetLyricIndex]?.text || "A Sonnet is Playing..."}
            </div>
          </div>
        </div>
      )}
