...
const Controls = ({ audio, hideNext , id=null}) => {
  ...
  const playNext = () => {
    ...
    if (index === -1 || index === allSongs.length - 1) {
      setCurrentSong(null); // Reset if no next song
      return
    }; // No next song
    ...
    if (nextSong) {
      ...
      audio.current.src = nextSong.audio; // Update audio source
  ...
  return (
    <div style={containerStyle}>
      ...
      {hideNext && (
        <button onClick={playNext} style={buttonStyle} title="Next song" aria-label="Next song">
          <FaForward />
        </button>
      )}
