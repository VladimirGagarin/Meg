...
const Controls = ({ audio, hideNext= true , id=null}) => {
  ...
  useEffect(() => {
   ...
   audioEl.addEventListener("stalled", onStalled, { once: true });
  ...
  return (
    <div style={containerStyle}>
      ...
      {(!hideNext && number < allSongs.length) && (
        <button
          onClick={playNext}
          style={{
            ...buttonStyle,
            backgroundColor: isSharing ? "#333" : "#444",
            cursor:
              isSharing || number === allSongs.length - 1
                ? "not-allowed"
                : "pointer",
            opacity: isSharing ? 0.5 : 1,
          }}
          ...
          disabled={
            !currentSongId || isSharing || number === allSongs.length - 1
          }
        >
          <FaForward />
        </button>
